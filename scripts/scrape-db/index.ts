...
const countryProms = geojson.features
  ...
  .map(async (country) => {
    const countryName =
      countryNameMap[country.properties.name] ?? country.properties.name;
    const mp = multiPolygon(
      country.geometry.type === "MultiPolygon"
        ? country.geometry.coordinates
        : ([country.geometry.coordinates] as any),
    );
    const b = bbox(mp, { recompute: true });
    const position = randomPosition(b);
    const req = await fetch(
      `https://www.taste.com.au/search-recipes/?q=${countryName}&ct=recipes`,
      {
    ...
    const $ = c.load(text);
    const resultsCount = Number.parseInt(
      $(".number-results-found .count").text(),
    );
    if (resultsCount === 0) {
      return undefined;
    }
...
const recipeProms = countryResults.map(async (recipe) => {
  ...
  const steps: string[] = [];
  $("ul.recipe-method-steps > li > .recipe-method-step-content").each(
    (_, e) => {
      steps.push($(e).text().trim());
    },
  );
  const ingredients: string[] = [];
  $(".recipe-ingredients-section > ul > li > .ingredient-description").each(
    (_, e) => {
      ingredients.push($(e).prop("data-raw-ingredient"));
    },
  );
...
await fs.promises.writeFile(
  ...
  JSON.stringify(await Promise.all(recipeProms), null, 4),
);
