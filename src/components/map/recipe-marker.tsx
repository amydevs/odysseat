...
export default function RecipeMarker({
  ...
  const screenCoordinates = React.useMemo(() => {
    ...
    return map.current.project(
      new LngLat(recipe.position[0], recipe.position[1]),
    );
  ...
  return (
    ...
    >
      <div
        className={cn(
          `relative h-16 w-16 cursor-pointer transition ease-in-out`,
          animate && "opacity-0",
          !expanded && "hover:scale-[1.15]",
        )}
        style={{
          animationDelay:
            markerDelay != undefined ? `${markerDelay}ms` : undefined,
      ...
      >
        ...
        >
          {recipe.thumbnailUrl && (
            <div
              className={cn(
                "absolute left-1/2 -translate-x-1/2 overflow-hidden transition-all duration-300",
                expanded
                  ? "top-1/2 h-40 w-60 -translate-y-26 rounded-3xl"
                  : "border-background top-1/2 h-16 w-16 -translate-y-1/2 rounded-4xl border-2",
              )}
              style={{
                transition:
                  "height 220ms, width 300ms, border-radius 300ms, translate 350ms",
          ...
          {expanded && (
            <Link
              ...
              className="text-foreground hover:text-muted-foreground absolute bottom-1 left-1/2 line-clamp-2 w-64 -translate-x-1/2 px-6 text-center text-base transition-all duration-100"
            >
              {recipe.title}
            </Link>
          )}
